...
FlatQuant的处理对象是大语言模型中所有线性层（包括自注意力模块和前馈网络中的投影层）的权重和激活值，其目标是处理Transformer架构中的每一个线性层的张量中存在的异常值。该方法提出了“平坦度”这一指标用于衡量张量是否易于量化，基于此构建了一个基于Kronecker分解和内核融合的量化框架，对权重和激活值的数据分布进行重塑，从而实现了INT4量化在精度和速度上的平衡和提升。

具体来说，FlatQuant为每个线性层学习一个最优的仿射变换矩阵 P，将输入数据映射到更易于量化的平坦分布，这一过程以平坦度为指标，目标函数为最小化量化后的均方误差。
